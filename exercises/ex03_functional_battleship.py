...
def input_guess(gridsize: int, row_col: str) -> int:
    ...
    if row_col == "row":
        ...
        while rguess > gridsize or rguess < 1:
            rguess = int(input(f"The grid is only {gridsize} by {gridsize}. Try again: "))
        return rguess
    if row_col == "column":
        cguess: int = int(input("Guess a column: "))
        while cguess > gridsize or cguess < 1:
            cguess = int(input(f"The grid is only {gridsize} by {gridsize}. Try again: "))
        return cguess
